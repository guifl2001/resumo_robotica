...
def processa_dist(frame_bgr, f, H):
    ...
    ret, y = cv2.threshold(frame_bgr[:,:,2], 230, 255, cv2.THRESH_BINARY) #isolando canal amarelo filtrando o canal vermleho e aplicando thresh baixo
    img = frame_bgr.copy()
    
    
    # isolating the green circle
    g_in = img[:,:,1]&(~img[:,:,2]) #filtrando verde pegando os canais verde e vermelho
    ...
    h = dist(centro_yellow,centro_green) #calcula distancia entre os centros dos circulos verde a amarelo 
